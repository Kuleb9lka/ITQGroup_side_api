...
public interface DocumentApi {
    ...
    @Operation(
            summary = "Check document approving safety",
            description = "Runs concurrent approval attempts for a document."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Execution completed successfully",
                    content = @Content(schema = @Schema(implementation = DocumentConcurrentResponseDto.class))),
            @ApiResponse(responseCode = "400", description = "Validation error"),
            @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    @PostMapping
    DocumentConcurrentResponseDto checkDocumentApprovingSafety(
            @RequestBody @Valid DocumentConcurrentRequestDto dto
    );


    @Operation(
            summary = "Batch create documents",
            description = "Reads quantity from file and creates documents."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Documents created successfully",
                    content = @Content(array = @ArraySchema(schema = @Schema(implementation = DocumentShortResponseDto.class)))),
